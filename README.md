...
| `EXTRACT_BACKEND` | — | `legacy` (default, локальний `renderer`) або `browser_agent` (зовнішній сервіс) |
| `BROWSER_AGENT_URL` | — | URL `browser-agent` (тільки коли `EXTRACT_BACKEND=browser_agent`) |
| `BROWSER_AGENT_API_KEY` | — | API-ключ `browser-agent` (header `X-API-Key`) |
| `PARSER_CALLBACK_BASE_URL` | — | Публічний URL цього `api`, на який BA шле callback з html |
| `PARSER_CALLBACK_SECRET` | — | Bearer-токен для перевірки автентичності callback'ів |
| `PARSER_WAIT_TIMEOUT_SEC` | — | Скільки чекати результат від BA (default 110, < бот 130с) |
| `PARSER_POLL_INTERVAL_SEC` | — | Інтервал GET `/api/tasks/:id` як fallback, якщо callback не дійшов (default 2с) |
| `PARSER_MAX_BYTES` | — | Обмеження `maxBytes` для get_page (default 500000) |
| `PARSER_FORCE_TIER` | — | Опційно: жорстко зафіксувати тир BA (`curl`/`playwright`/`browser`) |
...
### Альтернативний бекенд: `browser-agent` (`EXTRACT_BACKEND=browser_agent`)

Замість локального `renderer` фолбек може йти у зовнішній сервіс **browser-agent**, який
сам обирає тир (`curl → playwright → browser`) і має власну анти-детект інфраструктуру.

Потік:
1. Швидкий `httpx` GET у `api` — без змін, лишається первинним.
2. На `no_content` / `blocked` / 403·429·503 / timeout: `api` POST'ить у browser-agent
   `POST /api/run` (`generic/get_page`).
3. BA повертає `202 {task_id}`, виконує задачу і шле callback на
   `POST {PARSER_CALLBACK_BASE_URL}/internal/parser-callback` з `Authorization: Bearer
   {PARSER_CALLBACK_SECRET}` і `context.extractId`. Паралельно `api` робить poll
   `GET /api/tasks/{task_id}` як fallback на випадок втрати callback'у.
4. Якщо BA недосяжний (мережа/5xx) — `api` **автоматично сповзає на локальний `renderer`**,
   тож юзер усе одно отримує статтю. `depends_on: renderer` у docker-compose лишається.

`browser-agent` крутиться окремим стеком (інший репо). `api` спілкується з ним через
`BROWSER_AGENT_URL` (зовнішній HTTPS), а зворотний callback приходить через
`PARSER_CALLBACK_BASE_URL` (наприклад той самий cloudflared tunnel).

Перемикання — лише env, без перезбірки інших сервісів. Дефолт — `legacy`.
